...
export class Engine {
  ...
  private iterate() {
    ...
    for(let i = 0; i < this.ROWS; i++) {
      for(let j = 0 ; j <= this.COLUMNS ; j++) {
        const elements = [
          this.heightMapAverage[Math.min(i+1, this.ROWS)][j],
          this.heightMapAverage[Math.max(i-1, 0)][j],
          this.heightMapAverage[i][Math.max(j-1, 0)],
          this.heightMapAverage[i][Math.min(this.ROWS-1, j+1)],
          this.heightMapAverage[Math.min(i+1, this.ROWS)][Math.max(j-1, 0)],
          this.heightMapAverage[Math.min(i+1, this.ROWS)][Math.min(this.ROWS-1, j+1)],
          this.heightMapAverage[Math.max(i-1, 0)][Math.max(j-1, 0)],
          this.heightMapAverage[Math.max(i-1, 0)][Math.min(this.ROWS-1, j+1)],
        ]
        const value = .9 * (elements.reduce((total, num) => total + num) / 8) - this.heightMapAverage[i][j]
        this.heightMap[i][j] += value
        // this.heightMap[i][j] *= .9
      }
    }
    for(let i = 1; i < this.ROWS -1; i++) {
      for(let j = 1 ; j < this.COLUMNS - 1 ; j++) {
        this.heightMapAverage[i][j] += this.heightMap[i][j]
      }
    }
  ...
  private applyImpression(rowIndex, columnIndex) {
    this.heightMapAverage[rowIndex][columnIndex] = 100
  }
